...
function addOption(builder, option) {
    ...
    if (option.type === ApplicationCommandOptionType.Integer) {
        builder.addIntegerOption((item) => {
            item.setName(name).setDescription(description).setRequired(required);
            if (Number.isInteger(option.minValue)) item.setMinValue(option.minValue);
            if (Number.isInteger(option.maxValue)) item.setMaxValue(option.maxValue);
            return item;
        });
        return;
    }
...
function defaultPermissions(command) {
    if (["help", "ping", "userinfo"].includes(command.name)) return null;
...
function buildSlashPayload(commandCollection) {
    ...
    function push(name, command) {
        if (names.has(name) || !definitions[name]) return;
        names.add(name);
        ...
        if (perms) builder.setDefaultMemberPermissions(perms);
        for (const option of spec.options || []) addOption(builder, option);
...
async function registerGuildSlashCommands(client) {
    ...
    for (const guild of client.guilds.cache.values()) {
        ...
        console.log(`Slash: ${payload.length} commandes enregistrées sur ${guild.name}`);
    }
    return payload;
}
